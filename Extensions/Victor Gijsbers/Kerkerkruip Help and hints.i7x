...
Chapter - Contextual hints

[ Each hint has a variable. Once the conditions for the hint have been reached set the variable to 1. The hint rules will check for hints set to 1, display a message, and then set them to 2. This ensures that only one hint will be given per turn. ]

Section - The File of Hints
...
First when play begins:
	if File of Hints exists:
		read File Of Hints into Table of Hints;
...
Hint rule when hint-retreat is 1:
	...
	[TODO: fix bold italics here]
	say "[italic type][bracket]Note: when you first enter a room you may [bold type]retreat[italic type] for free.[close bracket][roman type][paragraph break]";

[ Remind the player to concentrate ]
...
Hint rule when hint-concentrate is 1:
	...
	say "[italic type][bracket]Note: don't forget to [bold type]concentrate[italic type]! Very few battles can be won without concentration. Check the [bold type]help[italic type] menu for more information about Kerkerkruip's combat system.[close bracket][roman type][paragraph break]";
